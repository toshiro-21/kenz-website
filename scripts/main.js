(function () {
  ...
  const sections = [
    ...
    { id: 'recipes', link: document.querySelector('.nav-link[href="#recipes"]') },
  ...
  const sectionIds = ['hero', 'products', 'story', 'usp', 'recipes', 'testimonials', 'contact'];
  ...
  function updateActiveNav() {
    ...
    navLinks.forEach((link) => {
      ...
      const isActive =
        (href === '#products' && (current === 'products' || current === 'hero')) ||
  ...
  // ——— 4. Newsletter form: validation + success message (no backend) ———
  if (newsletterForm) {
    newsletterForm.addEventListener('submit', (e) => {
      e.preventDefault();
      const email = newsletterEmail && newsletterEmail.value.trim();
      const valid = email && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
      if (!newsletterEmail || !newsletterSuccess) return;
      newsletterEmail.setCustomValidity(valid ? '' : 'Please enter a valid email address.');
      if (!valid) {
        newsletterEmail.reportValidity();
        return;
      }
      newsletterForm.classList.add('is-hidden');
      newsletterSuccess.classList.remove('visually-hidden');
